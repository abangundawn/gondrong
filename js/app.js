...
function renderMenu() {
  ...
  els.grid.innerHTML = localMenu.map(m => {
    ...
    return `<article ${articleClick} class="bg-white rounded-xl overflow-hidden card-pop flex flex-col h-full relative group ${cardClass}"><div class="relative h-40 w-full overflow-hidden bg-gray-200"><img src="${m.img}" onerror="this.src='https://placehold.co/300x200?text=No+Image'" class="w-full h-full object-cover transition duration-500 group-hover:scale-110">${isFullOOS ? '<div class="absolute inset-0 flex items-center justify-center bg-black/60 z-20"><span class="text-white font-black text-2xl border-4 border-white px-2 -rotate-12">HABIS!</span></div>' : ''}<div class="absolute top-2 right-2 bg-bebyte-purple text-white text-xs font-bold px-2 py-1 border-2 border-black rounded z-10">${m.category}</div>${m.nickname ? `<div class="absolute top-2 left-2 bg-black/70 text-white text-[10px] font-bold px-2 py-1 rounded z-10">${m.nickname}</div>` : ''}</div><div class="p-3 flex flex-col flex-grow"><h3 class="font-black text-lg text-black leading-tight uppercase">${displayName}</h3><p class="text-sm text-gray-500 mb-2 leading-tight min-h-[2.5em]">${m.desc || m.name}</p><div class="flex-grow"></div><div class="flex justify-between items-end mt-2 pt-2 border-t-2 border-dashed border-gray-200"><span class="font-bold text-bebyte-purple bg-purple-100 px-2 py-1 rounded border border-purple-200 text-sm">${fmt(m.price)}</span>${markerHtml}</div></div></article>`;
  }).join('');
}
